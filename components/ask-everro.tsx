...
export function AskEverro({ notificationBarVisible = false }: { notificationBarVisible?: boolean }) {
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} className={["fixed right-4 z-40 inline-flex items-center gap-2 rounded-2xl bg-charcoal px-4 py-3 text-sm font-semibold text-white shadow-xl transition hover:bg-signal-orange sm:right-6", notificationBarVisible ? "bottom-28 md:bottom-24" : "bottom-5"].join(" ")} aria-label="Ask Everro"><span className="text-signal-orange" aria-hidden="true">✦</span><span className="hidden sm:inline">Ask Everro</span><span className="sm:hidden">Ask</span></button>
    {open && <>
      <button type="button" aria-label="Close Ask Everro" onClick={() => setOpen(false)} className="fixed inset-0 z-50 bg-charcoal/30 backdrop-blur-[1px]" />
      <aside className="fixed inset-y-0 right-0 z-[60] flex w-full max-w-[520px] flex-col bg-white shadow-2xl sm:border-l sm:border-light-grey" aria-label="Ask Everro assistant">
        <header className="flex items-start justify-between gap-4 border-b border-light-grey px-5 py-5 sm:px-6"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-signal-orange">Opportunity intelligence</p><h2 className="mt-1 text-xl font-bold tracking-tight text-charcoal">Ask Everro</h2><p className="mt-1 text-xs leading-5 text-slate">Ask about buying signals, likely needs, timing and next actions across your marketplace.</p></div><button type="button" onClick={() => setOpen(false)} className="inline-flex h-9 w-9 items-center justify-center rounded-xl text-slate hover:bg-soft-surface hover:text-charcoal" aria-label="Close assistant">×</button></header>
