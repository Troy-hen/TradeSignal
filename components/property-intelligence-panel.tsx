      ...
      {intelligence ? (
        <section className="mt-6 overflow-hidden rounded-3xl border border-light-grey bg-white">
          <div className="flex flex-col gap-4 bg-soft-surface p-5 sm:flex-row sm:items-start sm:justify-between sm:p-6">
            ...
            {configured && (
              <button type="button" disabled={loading} onClick={() => enrich(true)} className="inline-flex shrink-0 items-center justify-center rounded-xl border border-light-grey bg-white px-3.5 py-2 text-xs font-semibold text-charcoal transition hover:border-signal-orange/40 hover:text-signal-orange disabled:opacity-50">
          ...
          <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[1.2fr_0.8fr]">
            <div>
              ...
              <ul className="mt-3 space-y-2">
                {intelligence.timing_reasons.map((reason) => <SignalLine key={reason}>{reason}</SignalLine>)}
              </ul>
        ...
        <section className="mt-6 rounded-3xl border border-light-grey bg-white p-5 sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-2 text-sm leading-6 text-slate">Match this project address to TwentyCI to surface property value, recent market activity, transaction recency and planning history. This enriches timing intelligence; it does not reveal private homeowner contact details.</p>
            </div>
            <button type="button" disabled={loading} onClick={() => enrich(false)} className="inline-flex shrink-0 items-center justify-center rounded-xl bg-charcoal px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-signal-orange disabled:cursor-not-allowed disabled:opacity-50">
          ...
          {epc ? (
            <>
              <div className="flex flex-col gap-4 bg-signal-orange/[0.04] p-5 sm:flex-row sm:items-start sm:justify-between sm:p-6">
                <div className="max-w-3xl">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-signal-orange">Energy intelligence</p>
                    <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-semibold text-charcoal">Official EPC data</span>
                  </div>
                  <h2 className="mt-2 text-xl font-bold tracking-tight text-charcoal">Energy-efficiency context for this property</h2>
                ...
                {epcConfigured && (
                  <button type="button" disabled={epcLoading} onClick={() => enrichEpc(true)} className="inline-flex shrink-0 items-center justify-center rounded-xl border border-signal-orange/20 bg-white px-3.5 py-2 text-xs font-semibold text-charcoal transition hover:border-signal-orange/50 hover:text-signal-orange disabled:opacity-50">
                    {epcLoading ? "Refreshing…" : "Refresh EPC"}
                  </button>
                )}
              </div>
              ...
              <dl className="grid gap-px bg-light-grey sm:grid-cols-2 xl:grid-cols-4">
                ...
                <Metric label="Property age" value={epc.construction_age_band ?? "Not available"} />
              </dl>

              <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[1.2fr_0.8fr]">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate">Commercially relevant energy signals</p>
                  {epc.improvement_signals.length > 0 ? (
                    <ul className="mt-3 space-y-2">{epc.improvement_signals.map((signal) => <SignalLine key={signal}>{signal}</SignalLine>)}</ul>
                  ) : (
                    <p className="mt-3 text-sm leading-6 text-slate">No strong upgrade signal was derived from the certificate. Keep the EPC as supporting property context rather than assuming improvement intent.</p>
                  )}
                  <p className="mt-4 text-xs leading-5 text-slate">EPC signals describe the recorded building and potential efficiency. They do not prove that the owner intends to buy a particular product or service.</p>
                </div>

                <div className="rounded-2xl border border-light-grey bg-soft-surface p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate">Building fabric & systems</p>
                  <dl className="mt-3 space-y-3 text-sm">
                    <SmallDetail label="Property type" value={[epc.property_type, epc.built_form].filter(Boolean).join(" · ") || "Not available"} />
            ...
            <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
              <div className="max-w-3xl">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-signal-orange">Energy intelligence</p>
                <h2 className="mt-2 text-xl font-bold tracking-tight text-charcoal">Check the property&apos;s EPC</h2>
                <p className="mt-2 text-sm leading-6 text-slate">Match the planning address against MHCLG Energy Performance Certificate data to add energy rating, building-fabric and heating context. MyTradeBox turns those facts into cautious trade-relevance signals.</p>
              </div>
              <button type="button" disabled={epcLoading} onClick={() => enrichEpc(false)} className="inline-flex shrink-0 items-center justify-center rounded-xl bg-signal-orange px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-charcoal disabled:cursor-not-allowed disabled:opacity-50">
                {epcLoading ? "Checking EPC…" : "Check EPC"}
              </button>
            </div>
          )}
...
function SignalLine({ children }: { children: React.ReactNode }) {
  return <li className="flex items-start gap-2 text-sm leading-6 text-charcoal"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-signal-orange" /><span>{children}</span></li>;
}
...
function SmallDetail({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0"><dt className="text-[10px] font-semibold uppercase tracking-[0.09em] text-slate">{label}</dt><dd className="mt-1 break-words font-semibold leading-5 text-charcoal">{value}</dd></div>;
}
...
function friendlyEpcError(error?: string) {
  if (error === "epc_not_matched") return "No sufficiently confident domestic EPC match was found for this project address.";
  if (error === "property_address_incomplete") return "This planning record does not contain enough address data to check an EPC.";
  if (error === "epc_auth_failed") return "The EPC API token needs to be refreshed in the deployment configuration.";
  if (error === "epc_rate_limited") return "The EPC service is temporarily rate-limiting requests. Try again shortly.";
