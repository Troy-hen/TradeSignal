  ...
  return (
    <div className={compact ? "rounded-2xl border border-light-grey bg-white p-4 text-charcoal shadow-[0_18px_50px_rgba(31,41,55,0.12)] sm:p-5" : "rounded-3xl border border-light-grey bg-white p-5 text-charcoal shadow-[0_18px_50px_rgba(31,41,55,0.08)] sm:p-7"}>
      ...
      <form onSubmit={handleSubmit} className={compact ? "mt-5 grid gap-3" : "mt-6 grid gap-3 sm:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)_auto]"}>
        <label className="block"><span className="mb-1.5 block text-[10px] font-bold uppercase tracking-[0.12em] text-slate">What do you sell?</span><textarea value={whatDoYouSell} onChange={(event) => setWhatDoYouSell(event.target.value)} placeholder="Describe your products or services in your own words" aria-label="What do you sell?" required maxLength={500} rows={compact ? 2 : 3} className="w-full resize-y rounded-xl border border-light-grey bg-white px-4 py-3 text-sm leading-6 text-charcoal placeholder:text-slate/70 focus:border-signal-orange focus:outline-none focus:ring-2 focus:ring-signal-orange/15" /></label>
