  ...
  return (
    <section id="coverage-shape" className="rounded-3xl border border-light-grey bg-white p-5 shadow-[0_18px_50px_rgba(31,41,55,0.04)] sm:p-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-signal-orange">2 · Geographic reach</p>
        ...
        <div className="rounded-2xl bg-soft-surface px-4 py-3 text-right">
          <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate">Selected plan</p>
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-3">
        {COVERAGE_PLANS.map((item) => <button key={item.id} type="button" onClick={() => choosePlan(item.id)} className={"rounded-2xl border p-4 text-left transition " + (selectedPlan === item.id ? "border-signal-orange bg-signal-orange/[0.06] shadow-sm" : "border-light-grey bg-soft-surface hover:border-signal-orange/40")}><div className="flex items-center justify-between gap-3"><span className="text-sm font-semibold text-charcoal">{item.name}</span>{selectedPlan === item.id && <span className="h-2 w-2 rounded-full bg-signal-orange" />}</div><p className="mt-3 text-2xl font-bold tracking-tight text-charcoal">{formatMonthlyGbp(item.monthlyPricePence)}<span className="ml-1 text-xs font-normal text-slate">/month</span></p><p className="mt-2 text-xs leading-5 text-slate">{item.description}</p></button>)}
      </div>
      ...
      <p className="mt-5 border-t border-light-grey pt-4 text-xs leading-5 text-slate">Selected: <span className="font-semibold text-charcoal">{plan.name} · {options.find((option) => option.id === mode)?.label}</span>. Existing subscriptions remain unchanged until a plan change is confirmed through billing.</p>
    </section>
  );
}
