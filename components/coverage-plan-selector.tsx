...
export function CoveragePlanSelector({ initialPlan = "local" }: { initialPlan?: CoveragePlanId }) {
  ...
  return (
    <section id="coverage-shape" className="rounded-3xl border border-light-grey bg-white p-6 shadow-[0_18px_50px_rgba(31,41,55,0.04)] sm:p-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-signal-orange">Compare your reach</p><h2 className="mt-2 text-2xl font-bold tracking-tight text-charcoal">The plan is geography, not a vertical.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate">Every plan opens the same intelligence engine. Your profile decides relevance; this comparison only changes how far the engine looks.</p></div><div className="rounded-2xl bg-signal-orange/10 px-4 py-3 text-right"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate">Lead unlocks</p><p className="mt-1 text-xl font-bold text-charcoal">{LEAD_UNLOCK_PRICE_GBP}</p><p className="text-[11px] text-slate">per opportunity</p></div></div>
      ...
      <div className="mt-6 flex flex-col gap-4 rounded-2xl bg-charcoal p-5 text-white sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold">{plan.name} · {options.find((option) => option.id === mode)?.label}</p><p className="mt-1 text-xs leading-5 text-white/60">Use this comparison to choose your preferred reach, then save your business profile below. Existing subscriptions remain unchanged until plan activation is connected.</p></div><a href="#profile" className="inline-flex shrink-0 items-center justify-center rounded-xl bg-signal-orange px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#e95f00]">Complete profile →</a></div>
    </section>
  );
}
