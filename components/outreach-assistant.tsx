...
export function OutreachAssistant({ opportunityId }: { opportunityId: string }) {
  ...
  function downloadDraft() {
    ...
    anchor.href = url;
    anchor.download = `everro-${channel}-draft.txt`;
    anchor.click();
  ...
  async function downloadPrintReadyLetter() {
    ...
    try {
      ...
      const anchor = document.createElement("a");
      anchor.href = url;
      anchor.download = match?.[1] ?? "everro-letter.pdf";
      anchor.click();
      URL.revokeObjectURL(url);
      setPostalMessage("Print-ready PDF created with a private Everro response link.");
    } catch {
      ...
      <div className="grid lg:grid-cols-[minmax(0,0.75fr)_minmax(0,1.25fr)]">
        <div className="bg-charcoal p-5 text-white sm:p-6">
          ...
          <p className="mt-3 text-sm leading-6 text-white/60">
            Everro drafts a first-contact angle from the evidence already on this brief. Nothing is sent automatically, and no applicant contact details are injected into the prompt.
          </p>
                ...
                <div className="mt-5 rounded-2xl border border-signal-orange/20 bg-signal-orange/[0.03] p-4">
                  ...
                  {postalEnabled && (
                    <>
                      ...
                      <p className="text-sm font-semibold text-charcoal">Print & post with Everro</p>
