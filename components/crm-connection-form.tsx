  ...
  return (
    <form action={formAction} className="mt-6 rounded-2xl border border-light-grey bg-white p-4 sm:p-5">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-xs leading-5 text-slate">Use an HTTPS endpoint from your CRM or automation layer. Everro sends a normalized purchased-lead payload and an idempotency key.</p>
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-[0.8fr_1.4fr_0.8fr_auto] md:items-end">
        ...
        <label className="text-xs font-semibold text-charcoal">Secret env var <span className="font-normal text-slate">(optional)</span><input name="secretRef" placeholder="CRM_WEBHOOK_SECRET" className="mt-2 w-full rounded-xl border border-light-grey bg-white px-3 py-2.5 text-sm font-normal focus:border-signal-orange focus:outline-none focus:ring-2 focus:ring-signal-orange/15" /></label>
      ...
      {state?.success && <p className="mt-3 text-sm text-success">Connection saved. It is ready to receive purchased leads.</p>}
    </form>
  );
}
