...
from category_seed;
with need_seed as (
  select m.market_slug,t.need_slug,t.name,t.description,t.category_group
  from _b2b_taxonomy_seed t cross join unnest(t.markets) as m(market_slug)
)
insert into public.need_categories(market_id,slug,name,description,category_group,is_default,is_active)
select om.id,ns.need_slug,ns.name,ns.description,ns.category_group,false,true
from need_seed ns join public.opportunity_markets om on om.slug=ns.market_slug
on conflict(market_id,slug) do update set name=excluded.name,description=excluded.description,category_group=excluded.category_group,is_active=true,updated_at=now();

with mapping as (
  select t.slug supplier_slug,m.market_slug,t.need_slug
  from _b2b_taxonomy_seed t cross join unnest(t.markets) as m(market_slug)
)
insert into public.supplier_need_mappings(supplier_category_id,need_category_id,match_weight,match_method,is_active)
select sc.id,nc.id,case when sc.category_group in ('software_technology','professional_advisory','finance_operations') then 0.82 else 0.76 end,'expanded_b2b_taxonomy',true
from mapping join public.supplier_categories sc on sc.slug=mapping.supplier_slug
...
on conflict(supplier_category_id,need_category_id) do update set match_weight=excluded.match_weight,match_method=excluded.match_method,is_active=true;
