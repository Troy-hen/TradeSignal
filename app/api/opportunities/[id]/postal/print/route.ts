...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const pdf = renderSimplePdf({
      ...
      subtitle: `Prepared with ${PRODUCT_BRAND.shortName} · ${new Date().toLocaleDateString("en-GB")}`,
      sections: [
        { heading: "To", lines: recipientLines },
        { heading: "", lines: finalContent.split(/\r?\n/) },
      ],
      footer: `Private project response link included · generated by ${PRODUCT_BRAND.shortName}`,
    });
...
function appendQuoteLink(content: string, url: string) {
  ...
  return `${content.trim()}\n\nWant to discuss the project or request a quote?\n${url}\n\nUse this private ${PRODUCT_BRAND.shortName} response link to call, WhatsApp or request a quote directly.`;
}
