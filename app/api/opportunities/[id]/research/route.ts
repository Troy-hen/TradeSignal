...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: reportRow, error: insertError } = await db
    .from("opportunity_research_reports")
    .insert({ company_id: company.id, opportunity_id: id, status: "running", generated_by: user.id, model: process.env.ASK_EVERRO_MODEL ?? process.env.ASK_TRADESIGNAL_MODEL ?? process.env.ASK_MYTRADEBOX_MODEL ?? "gpt-5-mini" })
    .select("id")
    .single();
