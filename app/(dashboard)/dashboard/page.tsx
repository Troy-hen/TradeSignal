...
export default async function DashboardPage() {
  ...
  return (
    <div className="min-w-0 space-y-8">
      ...
      <section className="grid min-w-0 gap-8 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="min-w-0"><AppSectionHeader eyebrow="Marketplace feed" title="Start with what is new." description="Each card keeps the decision simple: why now, likely need, then unlock if the fit is clear." action={<Link href="/opportunities" className="text-sm font-semibold text-signal-orange">View all →</Link>} />{newItems.length === 0 && newSignals.length === 0 ? <div className="mt-5 rounded-3xl border border-dashed border-light-grey bg-white p-8 text-center text-sm leading-6 text-slate">No new opportunities are waiting. Adjust the Marketplace filters or review your saved shortlist.</div> : <div className="mt-5 grid gap-5 md:grid-cols-2">{newItems.slice(0, 2).map((item) => <OpportunityRow key={item.leadMatchId} item={item} unlocked={paidOpportunityIds.has(item.opportunityId)} />)}{newSignals.slice(0, 2).map((item) => <MarketSignalRow key={item.market_signal_trade_match_id} item={item} unlocked={paidMarketSignalIds.has(item.market_signal_trade_match_id)} />)}</div>}</div><aside className="min-w-0 rounded-3xl border border-light-grey bg-white p-6 shadow-[0_14px_45px_rgba(31,41,55,0.045)]"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-signal-orange">Next best move</p><h2 className="mt-3 text-2xl font-bold tracking-tight text-charcoal">Qualify one strong opportunity.</h2><p className="mt-3 text-sm leading-6 text-slate">Choose the opportunity with the clearest buying window and unlock the full brief only when it earns the £20.</p><div className="mt-6 space-y-3 border-t border-light-grey pt-5 text-sm"><LightRow label="Hot now" value={String(hotCount)} /><LightRow label="Shortlisted" value={String(savedCount)} /><LightRow label="Purchased leads" value={String(paidUnlocks.length)} /></div><Link href="/opportunities?action=saved" className="mt-6 inline-flex text-sm font-semibold text-signal-orange">Review saved leads →</Link></aside></section>
    </div>
  );
}
