  ...
  return (
    <div className="min-w-0 space-y-8">
      <AppPageHeader
        ...
        actions={<><Link href="/opportunities" className="inline-flex items-center justify-center rounded-xl bg-signal-orange px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#e95f00]">Open marketplace <span className="ml-2">→</span></Link><Link href="/territories" className="inline-flex items-center justify-center rounded-xl border border-light-grey px-5 py-3 text-sm font-semibold text-charcoal transition hover:border-charcoal/25 hover:bg-soft-surface">Open map</Link></>}
        stats={[{ label: "Open opportunities", value: String(openCount), detail: "Ready to qualify" }, { label: "Hot now", value: String(hotCount), detail: "Highest current fit" }, { label: "Purchased leads", value: String(paidUnlocks.length), detail: "Permanent access" }, { label: "Saved", value: String(savedCount), detail: "Shortlisted to revisit" }]}
      />
