    ...
    <div className="min-w-0 space-y-8">
      <section className="grid min-w-0 gap-6 overflow-hidden rounded-[2rem] bg-charcoal p-6 text-white shadow-xl shadow-charcoal/10 sm:p-9 lg:grid-cols-[minmax(0,0.9fr)_minmax(360px,1.1fr)] lg:items-center">
        <div className="min-w-0">
          ...
          <p className="mt-4 max-w-xl text-sm leading-7 text-white/65 sm:text-base">Your coverage and business profile turn signals from every enabled source into a focused opportunity surface: why now, fit, evidence and next move.</p>
          <div className="mt-7 flex flex-wrap gap-3">
            ...
            <Link href="/coverage" className="inline-flex items-center justify-center rounded-xl border border-white/15 px-5 py-3 text-sm font-semibold text-white/85 transition hover:border-white/35 hover:bg-white/5">Manage coverage</Link>
          </div>
        </div>

        <div className="min-w-0 rounded-3xl border border-white/10 bg-white/[0.07] p-4 sm:p-5">
          <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-white/45">Your live coverage</p><p className="mt-1 text-lg font-semibold text-white">Fresh signals worth reviewing</p></div><span className="rounded-full bg-success/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.1em] text-success">Coverage active</span></div>
          <div className="mt-5 space-y-3">
            ...
            {opportunities.length === 0 && marketSignals.length === 0 && <p className="rounded-2xl border border-dashed border-white/15 p-6 text-center text-sm text-white/55">Your coverage is ready. New relevant signals will appear here as they are qualified.</p>}
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-signal-orange">Signal pulse</p><h2 className="mt-2 text-2xl font-bold tracking-tight text-charcoal">Where your current opportunities are coming from.</h2></div><Link href="/markets" className="text-sm font-semibold text-signal-orange">View intelligence sources →</Link></div>
