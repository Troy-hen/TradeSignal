...
import { CrmConnectionManager } from "@/components/crm-connection-manager";
...
export default async function SettingsPage() {
  ...
  const [{ data: companyRow }, { data: prefs }, connections, alertRules] = await Promise.all([
    ...
    listCrmConnections(company.id, canEdit),
...
function CrmConnections({ canEdit, connections, mappingsByConnection }: { canEdit: boolean; connections: CrmConnection[]; mappingsByConnection: Map<string, CrmFieldMapping[]> }) {
  const connectors = [
    { name: "HubSpot", detail: "Company, contact and deal handoff with evidence attached.", available: false },
    { name: "Pipedrive", detail: "Organisation, person and deal creation for purchased leads.", available: false },
    { name: "Generic webhook", detail: "Send a controlled opportunity payload into another CRM or workflow.", available: true },
  ];
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-3">
          {connections.map((connection) => (
            <article key={connection.id} className="rounded-2xl border border-light-grey bg-white p-4">
              ...
              {canEdit && connection.provider === "generic_webhook" ? <CrmConnectionManager connection={connection} /> : <p className="mt-4 rounded-xl border border-light-grey bg-soft-surface px-3 py-2 text-center text-xs font-semibold text-slate/70">{canEdit ? "Provider controls are added when the adapter is enabled" : "Admin access required"}</p>}
            </article>
          ))}
        </div>
      )}
      ...
      <div className="mt-6 grid gap-3 lg:grid-cols-3">
        {connectors.map((connector) => (
          <article key={connector.name} className="rounded-2xl border border-light-grey bg-white p-4">
            <div className="flex items-center justify-between gap-3"><h3 className="text-sm font-semibold text-charcoal">{connector.name}</h3><span className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate">{connections.some((connection) => providerLabel(connection.provider) === connector.name) ? "Configured above" : connector.available ? "Available" : "Planned"}</span></div>
            <p className="mt-2 text-xs leading-5 text-slate">{connector.detail}</p>
            <p title={canEdit ? (connector.available ? "Configure an HTTPS endpoint, then test it above" : "This adapter will be enabled after its provider API and credential flow are complete") : "Only company owners and admins can connect a CRM"} className="mt-4 rounded-xl border border-light-grey bg-soft-surface px-3 py-2 text-center text-xs font-semibold text-slate/70">{canEdit ? connector.available ? "Configure in this page" : "Provider adapter planned" : "Admin access required"}</p>
          </article>
        ))}
      </div>
      <div className="mt-5 rounded-2xl border border-white bg-white/80 p-4"><p className="text-sm font-semibold text-charcoal">Connection controls stay out of the lead workflow.</p><p className="mt-1 text-xs leading-5 text-slate">Everro sends a normalized lead payload with stable field names and an idempotency key. Test, edit or disconnect an endpoint here; field mappings are stored per connection and provider adapters can be added without changing the Purchased workspace.</p></div>
    </section>
  );
}
