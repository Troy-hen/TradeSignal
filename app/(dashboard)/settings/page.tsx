...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ coverage_plan?: string; coverage_mode?: string; focus?: string }> }) {
  ...
  return (
    <div className="min-w-0 space-y-8">
      <AppPageHeader eyebrow="Account" title="Business profile." description="Keep your company details, customer profile and opportunity alerts up to date. This context helps the intelligence engine decide what is relevant." actions={<Link href="/opportunities" className="inline-flex items-center justify-center rounded-xl border border-light-grey px-4 py-2.5 text-sm font-semibold text-charcoal transition hover:border-charcoal/25 hover:bg-soft-surface">Open marketplace <span className="ml-2">→</span></Link>} />
