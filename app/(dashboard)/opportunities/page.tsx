...
export default async function OpportunitiesPage({ searchParams }: { searchParams: Promise<{ bucket?: string; action?: string; source?: string }> }) {
  ...
  const visiblePlanning = planning.filter((item) => !paidOpportunityIds.has(item.opportunityId));
  ...
  const openCount = [...visiblePlanning.map((item) => item.currentAction), ...visibleMarketSignals.map((item) => item.current_action)].filter((value) => !["won", "lost"].includes(value ?? "")).length;
  ...
  return (
    <div className="min-w-0 space-y-8">
      ...
      {totalCount === 0 ? <EmptyState hasCoverage={hasCoverage} filtered={filtered} savedView={savedView} /> : <div className="space-y-8">{visiblePlanning.length > 0 && <section><div className="mb-4 flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-signal-orange">{savedView ? "Saved business opportunities" : "Business opportunities"}</p><h2 className="mt-1 text-2xl font-bold tracking-tight text-charcoal">{savedView ? "Your shortlisted buying windows." : "Signals matched to your profile."}</h2></div><span className="text-xs text-slate">{planning.length}</span></div><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{visiblePlanning.map((item) => <OpportunityRow key={item.leadMatchId} item={item} unlocked={paidOpportunityIds.has(item.opportunityId)} />)}</div></section>}{visibleMarketSignals.length > 0 && <section><div className="mb-4 flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-signal-orange">{savedView ? "Saved public signals" : "Additional intelligence"}</p><h2 className="mt-1 text-2xl font-bold tracking-tight text-charcoal">{savedView ? "Signals you chose to keep close." : "Public, commercial and procurement signals."}</h2></div><span className="text-xs text-slate">{visibleMarketSignals.length}</span></div><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{visibleMarketSignals.map((item) => <MarketSignalRow key={item.market_signal_trade_match_id} item={item} unlocked={paidMarketSignalIds.has(item.market_signal_trade_match_id)} />)}</div></section>}</div>}
    </div>
  );
}
...
function EmptyState({ hasCoverage, filtered, savedView }: { hasCoverage: boolean; filtered: boolean; savedView: boolean }) {
  ...
  const body = savedView ? "Save an opportunity from the marketplace and it will appear here for follow-up. Leads you have already unlocked live in Purchased leads." : hasCoverage ? filtered ? "Clear the filters to return to the complete relevant feed." : "New relevant opportunities will appear here as the intelligence engine qualifies them." : "Tell us what you sell and where you operate. Geography controls reach; the profile controls relevance.";
  return <div className="rounded-3xl border border-dashed border-light-grey bg-white p-10 text-center sm:p-14"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-signal-orange/10 text-xl text-signal-orange">{savedView ? "♡" : "✦"}</div><h2 className="mt-5 text-lg font-semibold text-charcoal">{title}</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate">{body}</p>{savedView ? <div className="mt-5 flex flex-wrap justify-center gap-3"><Link href="/opportunities" className="inline-flex rounded-xl bg-signal-orange px-4 py-2.5 text-sm font-semibold text-white">Browse marketplace →</Link><Link href="/purchased" className="inline-flex rounded-xl border border-light-grey px-4 py-2.5 text-sm font-semibold text-charcoal">Purchased leads</Link></div> : hasCoverage && filtered ? <Link href="/opportunities" className="mt-5 inline-flex rounded-xl bg-charcoal px-4 py-2.5 text-sm font-semibold text-white">Clear filters</Link> : !hasCoverage ? <Link href="/coverage" className="mt-5 inline-flex rounded-xl bg-signal-orange px-4 py-2.5 text-sm font-semibold text-white">Set up coverage →</Link> : null}</div>;
}
