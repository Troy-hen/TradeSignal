...
async function PaidBrief({ opportunity, companyId, unlockId }: { opportunity: Opportunity; companyId: string; unlockId: string }) {
  ...
  return (
    <div className="min-w-0 space-y-6">
      ...
      <details className="group rounded-3xl border border-light-grey bg-white"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-5 sm:p-7"><span><SectionKicker>Work this lead</SectionKicker><span className="mt-2 block text-xl font-bold tracking-tight text-charcoal">Set the stage and next follow-up.</span><span className="mt-1 block text-sm text-slate">Keep lead management close without crowding the brief. CRM handoff is always available above.</span></span><span className="rounded-xl bg-soft-surface px-3 py-2 text-xs font-semibold text-slate transition group-open:rotate-180">⌄</span></summary><div className="space-y-5 border-t border-light-grey p-5 sm:p-7">{leadMatchId && <LeadActionPanel leadMatchId={leadMatchId} currentAction={currentAction} />}{leadMatchId && <FollowUpPanel leadMatchId={leadMatchId} opportunityId={opportunity.id} initialFollowUps={followUps} />}</div></details>
