...
export async function retrieveKnowledge(query: string, limit = 6): Promise<KnowledgeHit[]> {
  ...
  if (openai) {
    try {
      ...
      console.warn("Ask Everro semantic retrieval unavailable; falling back to full-text retrieval", error);
    }
  }
  ...
  if (error) {
    console.error("Ask Everro knowledge retrieval failed", error);
    return [];
  }
...
export async function searchOpportunityTeasers({
  ...
  if (error) {
    console.error("Ask Everro opportunity retrieval failed", error);
    return [];
  }
