...
export async function getWorkspaceSnapshot(companyId: string) {
  ...
  const [{ data: claims }, { data: followUps }, { data: nearby }] = await Promise.all([
...
function priorityScore(item: Awaited<ReturnType<typeof getCompanyOpportunities>>[number]) {
...
function toAssistantOpportunity(item: Awaited<ReturnType<typeof getCompanyOpportunities>>[number]) {
  return {
