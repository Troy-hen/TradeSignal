 ...
 */
export async function listCrmConnections(companyId: string): Promise<CrmConnection[]> {
  const client = (await createClient()) as unknown as LooseClient;
  const { data, error } = await run<CrmConnection[]>(
    client
      .from("crm_connections")
      .select(
        "id, company_id, provider, label, status, external_account_id, external_account_name, scopes, last_connected_at, last_synced_at, last_error, updated_at",
      )
